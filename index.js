...
const sendVeficationEmail = require('./helpers/email/emailSend');
...
app.post("/registration", async (req, res) => {
  ...
  try {
    ...
    console.log(userExists, 'chekc');

    if (userExists) {
      return res.status(400).json({ error: "User already registered" });
    }
    const { _id, name, email: getMail } = await Users.create(req.body);
    let userid = _id;
    const verificationLink = `http://api.wilteamfive.com/verifi-link/${userid}`; // Generate a real token and link

    const testing = await sendVeficationEmail({ name: name, receiver: getMail, subject: "Email Verification", link: verificationLink });
    console.log(testing, 'testing  this email');


    res.status(201).json({ data: userid, link: verificationLink });
  } catch (error) {
...
app.get('/verifi-link/:user_id', async (req, res) => {
  const userId = req.params.user_id;
  console.log(userId, 'coming id');

  try {
    const user = await User.findByIdAndUpdate(userId, { $set: { disable: false } }, { new: true });

    if (!user) {
      return res.status(404).json({ message: 'User not found', status: 404 });
    }

    res.status(200).json({ data: user, message: 'Verification successful', status: 200 });
  } catch (error) {
...
// -------------------------------- delete user all request's-------------------------------------

app.delete('/delete-user/:userId', async (req, res) => {
  const _userId = req.params.userId;
  try {
    const deleteUser = await Users.findByIdAndDelete(_userId);
    console.log(deleteUser, "user delete");

    if (deleteUser) {
      return res.json({ message: "User delete successfully", status: 200 });
    } else {
      return res.json({ message: "User is not found", status: 400 });
    }
  } catch (error) {
    return res.json({ message: "Somethign went wrong", status: 500 });
  }
});
